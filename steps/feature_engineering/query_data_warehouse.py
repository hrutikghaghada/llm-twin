...
from llm_twin.domain.documents import ArticleDocument, Document, RepositoryDocument, UserDocument
...
def _fetch_articles(user_id) -> list[NoSQLBaseDocument]:
    return ArticleDocument.bulk_find(author_id=user_id)


def _fetch_repositories(user_id) -> list[NoSQLBaseDocument]:
